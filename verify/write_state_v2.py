import sys, io
bal = sys.argv[1] if len(sys.argv) > 1 else 'see `higgsfield account status`'
new = '''# The Tower Energy & Nutrition — scroll-film site — STATE (HEAD file; read this first)
...
Balance 241.1 at the start → **''' + bal + '''** now.
...
'''
io.open('STATE.md', 'w', encoding='utf-8').write(new)
print('STATE.md written')
