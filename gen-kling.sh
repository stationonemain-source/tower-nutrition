...
echo "$RAW" > clips/float/$NAME.json
...
curl -fsSL -o clips/float/$NAME.mp4 "$URL" || exit 1
echo "[$NAME] clip: $(ffprobe -v error -select_streams v -show_entries stream=width,height,nb_frames,r_frame_rate -of csv=p=0 clips/float/$NAME.mp4)"
echo "[$NAME] background remover..."
RAW2=$(higgsfield generate create video_background_remover --video-references clips/float/$NAME.mp4 --wait --wait-timeout 20m --wait-interval 6s --json 2>&1)
echo "$RAW2" > clips/float/$NAME-alpha.json
...
curl -fsSL -o clips/float/$NAME-alpha.$EXT "$URL2" || exit 2
echo "[$NAME] alpha: $EXT $(ffprobe -v error -select_streams v -show_entries stream=codec_name,pix_fmt,width,height,nb_frames -of csv=p=0 clips/float/$NAME-alpha.$EXT)"
